...
async def chat_endpoint(req: ChatRequest, request: Request, background_tasks: BackgroundTasks):
    ...
    # Safe memory retrieval – fallback to empty if error
    memory_context = ""
    try:
    ...
    try:
        ...
        response = "I'm having trouble connecting to AI services. Please try again later."
        model = "fallback"
...
async def chat_stream(req: ChatRequest, request: Request, background_tasks: BackgroundTasks):
    ...
    async def generate():
        ...
        try:
            ...
            # Store the complete response (or error) in DB

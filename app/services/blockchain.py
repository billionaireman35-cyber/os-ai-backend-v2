...
def get_all_balances(address: str) -> dict:
    result = {}
    for chain_name in settings.SUPPORTED_CHAINS:
        try:
            web3 = get_web3(chain_name)
            native_balance = web3.eth.get_balance(address)
            native_symbol = CHAINS.get(chain_name, {}).get("symbol", chain_name.upper())
            result[chain_name] = {
                "native": {
            ...
            # Token list – for now only Polygon has known tokens
            token_list = []
            if chain_name == "polygon":
                token_list = [
                    {"address": settings.CLOSE_CONTRACT_ADDRESS, "symbol": "CLOSE", "decimals": 18},
